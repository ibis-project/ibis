...
We've made our cube and we've made it walk, but now we want to make it talk. At
this point, you might be questioning the utility of this blog post -- what are we
doing and why? The purpose of this blog post is to demonstrate generating
synthetic data that can look realistic. We achieve this be building in
randomness (e.g. a random walk can be used to simulate stock prices) and also by
using that randomness to inform the generation of more interesting synthetic
data (e.g. the number of shares being bid on).
...
fake.name(), fake.sentence(), fake.location_on_land()
```
...
schema. You can apply this to match the schema and expected distributions of
...
mentioning AI. [In our next post, we use open source language models to generate
more realistic text](../os-lms-for-data/index.qmd).

## Next steps

If you've followed along, you have a `synthetic.ddb` file with a few tables:
...
While this isn't much data, hopefully you can build on this to generate
realistic synthetic data at any scale for any use case.
