...
We need to aggregate the features and collect statistics based on `case_id`
then join with the base table to use them as features
```python
ibis.read_parquet(TRAIN_DIR / "train_credit_bureau_b_1.parquet") \
.relocate("num_group1").order_by(["case_id", "num_group1"]).head()
```
...
chaining all transformation steps, which will be applied to the train and test datasets later.
...
IbisML recipes in various formats, making it compatible with various modeling frameworks.
The default output format is `Numpy Array`
...
nn_classifier = SimpleNeuralNetClassifier(
...
We'll build an xgboost classifier
