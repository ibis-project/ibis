...
The remaining files contain features, consisting of approximately 465 features from
...
```
### Select features
Since there are 465 features, we'll only select part of good ones for this post:
```{python}
...
We set up a pipeline that includes two components:
...
Force the garbage collection to release unreferenced memory.
```{python}
import gc
...
Define a neural network classifier using pytorch and pytorch lighting:
```{python}
import numpy as np
import torch
