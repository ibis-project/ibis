...
compare a few modern query engines, explore the Ibis API as a great choice for
each of them, and discuss the results.
...
joins.  SQL queries are defined by the TPC-H benchmark. We run the SQL queries
and equivalent dataframe code via Ibis and Polars APIs.
...
to get a sense of any overhead in Ibis dataframe code. Note that Ibis dataframe
code generates SQL for the DuckDB and DataFusion backends and generates
dataframe code for the Polars backend.
...
no streaming engine** (per the Polars team's recommendation, as the current
streaming engine is being rewritten). This also allows us to compare the
performance of the Polars backend through Ibis with the Polars API directly for
the queries that do succeed.
...
Interesting in contributing to Ibis? Increasing coverage of operations for a
backend is a great place to start!
:::
...
that's best for your use case. If you newer, better engine comes along you'll
likely be able to use that too. And you can scale up and out across the 20+
backends Ibis supports as needed.
...
given system. You should favor the system that works best for you.
