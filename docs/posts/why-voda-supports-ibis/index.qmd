...
- Big changes are afoot in the world of computing hardware. [Moore’s Law]
  (https://en.wikipedia.org/wiki/Moore%27s_law) is coming to an end.
...
- Wouldn’t it be nice if there was a composable data processing engine that
  could be embedded into _any_ data platform, to enable that platform to run
  jobs on accelerated hardware? Then the platform builders wouldn’t need to
  duplicate efforts. And organizations wouldn’t need to migrate to different
  platforms to accelerate their workloads.
...
- Theseus is great for very large-scale ETL workloads with very high throughput
  ...
  wants to make it easy to choose the best engine for your workload. Your
  choice of engine or platform should not limit your choice of other tools.
