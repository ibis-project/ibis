...
backends today, including pandas and DuckDB and Polars and Postgres and BigQuery
and Snowflake and PySpark and over a dozen more.
...
hit OOM errors and learned about how much data expands in memory and why to
store files in compressed Parquet. I learned about partitioning strategies and
the joy of subtle differences and incompatibilities across data system. I
started using PySpark and Dask to overcome hurdles. I slowly learned about
databases. Fortunately while I've been learning, others ([including many Ibis
contributors](https://github.com/ibis-project/ibis/contributors)) have been
building a modern, open-source Python data ecosystem! Now I can just use what
they've built.

In this blog, we'll look at how to build an end-to-end analytics project using
Ibis as the frontend for data in Python. Ibis plays a large part, but is only
one component in any real data project. 
...
real business value at little to no cost.
...
opportunity to try out a few more.
...
::: {.callout-warning}
While I consider this "production" it does go occasionally, usually because of a
stale connection to MotherDuck. There are screenshots below in case that happens
while you're reading this, or you can reproduce it locally (an exercise for the
reader).
:::
...
cloud service for DuckDB. This allows us to use MotherDuck's serverless servers
to access the production data from anywhere, including my laptop and a Streamlit
Community Cloud server. The dashboard deployed there updates automatically when
the data is updated. Finally, we can run some tests by executing the dashboard code.
...
and various environment variables set to access the data sources and cloud
services. These are stored in a `.env` file or set in the respective cloud
services (GitHub Actions secrets and Streamlit Community Cloud secrets).
...
same code locally or in a GitHub Action. This currently includes:
...
### Directed acyclic graph (DAG) and input/ouput management
...
backends. I define the following table IO managers and use the `DuckDBIOManager`
...
While the GitHub (ommitted for brevity) and Zulip data is in JSON format:
...
We run a postprocessing script to combine the separate data into a single DuckDB database:
...
This also backs up the data as Delta Lake tables for good measure.

After this, our data directory looks like:
...
The EDA makes it easy to iteratively define metrics and plot the data. Once I've settled on the code I want, I add it to `metrics.py` with sufficient comments to make it appear in the Streamlit dashboard.
...
I found thinking about my key metrics and how I want them to be displayed (and
documented with code comments) in a single Python file refreshing.
