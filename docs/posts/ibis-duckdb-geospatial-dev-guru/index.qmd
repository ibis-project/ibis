...
data is also available from the citibike [source](https://s3.amazonaws.com/tripdata/index.html) but as a `.csv` file.
...
biketrip = biketrip.mutate(linear_distance = trip_distance, efficiency_ratio = biketrip.tripduration / trip_distance)
...
trips_within_500 = biketrip.filter(_.pickup_point.within(
    first_point.select(_.pickup_point.buffer(500)).to_array()))
