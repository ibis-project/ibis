...
Ibis defines a Python dataframe API that supports any query engine -- the
frontend for any backend data platform. This includes 20+ backends today, most
SQL-based.  This allows Ibis to have excellent performance -- as good as the
backend it is connected to -- with a consistent user experience.

We can demonstrate this with a simple example on a few local backends:
...
Ibis for data engineers, data analysts, and data scientists to use directly with
their data platform(s) of choice. It also has benefits for [data
platforms](#ibis-for-data-platforms),
[data organizations](#ibis-for-data-organizations), and
[data library developers](#ibis-for-data-library-developers).

### Ibis for data practitioners
...
This third step is where Ibis comes in. Instead of spending X money and Y time
developing a bespoke Python dataframe library, you can create an Ibis backend
for your data platform [in as little as four hours for an experienced Ibis
...
[Google BigQuery
DataFrames](https://github.com/googleapis/python-bigquery-dataframes) are a more
...
### Ibis for data organizations

Data organizations can use Ibis to standardize the interface for SQL and Python
data practitioners. It also allows them to:
...
### Ibis for data library developers

Python developers creating data libraries can use Ibis to:
...
# Function to add new backends to a category
...
category_colors = {
    "Ibis API": "#999999",  # Grey for neutrality.
    "Naïve execution": "#FF8C00",  # A bright, deep orange.
    "Expression-generating": "#6A5ACD",  # A slate blue, offering a clear contrast with orange.
    "SQL-generating": "#3CB371",  # A medium sea green, vibrant yet distinct from blue and orange.
}
...
def create_nodes_and_links():
    ...
    for category, backends in backend_categories.items():
        ...
        for backend in backends:
            if (
                backend not in node_index
            ):  # Add the backend if it's not already in the index
                nodes.append({"label": backend, "color": category_colors[category]})
                node_index[backend] = idx
                idx += 1
            # Link from category to backend
            links.append(
                {
...
# Update layout for more padding around the figure
...
As you can see, most backends generate SQL strings. Ibis uses
[SQLGlot](https://github.com/tobymao/sqlglot) to transform Ibis expressions into
SQL strings. You can also use the `.sql()` methods to mix in SQL strings,
