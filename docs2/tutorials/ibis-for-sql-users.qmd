...
stats = events.group_by(keys).aggregate(total=events.count(), sessions=sessions)
```
...
direction of any key, wrap it in `ibis.desc`:


```{python}
sorted = events.order_by([ibis.desc("event_type"), ibis.desc(events.ts.month())]).limit(
    100
)
...
```{python}
case = (
    ibis.case().when(t.two < 0, t.three * 2).when(t.two > 1, t.three).else_(t.two).end()
)
...
```{python}
t1 = ibis.table([("value1", "float"), ("key1", "string"), ("key2", "string")], "table1")

t2 = ibis.table([("value2", "float"), ("key3", "string"), ("key4", "string")], "table2")
```
