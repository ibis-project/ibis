...
in (lib.optionalAttrs stdenv.hostPlatform.isDarwin {
  pyproj = prev.pyproj.overrideAttrs (attrs: {
    nativeBuildInputs = attrs.nativeBuildInputs or [ ]
      ++ [ final.setuptools final.cython pkgs.proj ];
    PROJ_DIR = "${lib.getBin pkgs.proj}";
    PROJ_INCDIR = "${lib.getDev pkgs.proj}";
  });
}) // lib.mapAttrs (name: spec: addBuildSystems prev.${name} spec)
buildSystemOverrides // {
  ...
  pandas = prev.pandas.overrideAttrs (attrs:
    ...
    (stdenv.isAarch64 && stdenv.isLinux && shortVersion == "310") {
  ...
  mysqlclient = prev.mysqlclient.overrideAttrs (attrs: {
    ...
    buildInputs = attrs.buildInputs or [ ]
      ++ [ pkgs.pkg-config pkgs.libmysqlclient ];
  });
  ...
  pyodbc = prev.pyodbc.overrideAttrs
    (attrs: { buildInputs = attrs.buildInputs or [ ] ++ [ pkgs.unixODBC ]; });
  ...
  pyspark = prev.pyspark.overrideAttrs (attrs:
    let
      ...
      scalaVersion = "2.12";
      jarName =
        "iceberg-spark-runtime-${pysparkVersion}_${scalaVersion}-${icebergVersion}.jar";
      icebergJarUrl =
        "https://search.maven.org/remotecontent?filepath=org/apache/iceberg/iceberg-spark-runtime-${pysparkVersion}_${scalaVersion}/${icebergVersion}/${jarName}";
    ...
    in {
      ...
      postInstall = attrs.postInstall or "" + ''
        ...
        cp -v ${
          ../docker/spark-connect/log4j2.properties
        } $out/${final.python.sitePackages}/pyspark/conf/log4j2.properties
      '';
    });
