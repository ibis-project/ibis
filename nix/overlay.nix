...
let
  ...
  defaultDeps = {
    ibis-framework =
      [ "duckdb" "datafusion" "sqlite" "polars" "decompiler" "visualization" ];
  };
  ...
  mkEnv' = {
        ...
        (pkgs.callPackage pyproject-nix.build.packages {
          ...
          stdenv = stdenv.override {
            targetPlatform = targetPlatform // {
              darwinSdkVersion =
                if targetPlatform.isAarch64 then "14.0" else "12.0";
            };
          };
    ...
    in (pythonSet.mkVirtualEnv "ibis-${python.pythonVersion}"
      deps).overrideAttrs (_old: {
...
in {
  ...
  lychee = super.lychee.overrideAttrs (attrs:
    lib.optionalAttrs
  ...
  ibisSmallDevEnv = mkEnv' {
    deps = { ibis-framework = [ "dev" ]; };
    editable = false;
  } pkgs.python313;

  duckdb = super.duckdb.overrideAttrs (_:
  ...
  get-latest-quarto-hash = pkgs.writeShellApplication {
    ...
    runtimeInputs = [ pkgs.nix pkgs.gh pkgs.jq ];
    text = ''
