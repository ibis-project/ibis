...
class TestClickhouseClient(ClickhouseE2E, unittest.TestCase):
    ...
    def test_run_sql(self):
        query = 'SELECT * FROM {0}.functional_alltypes'
        table = self.con.sql(query.format(self.test_data_db))

        fa = self.con.table('functional_alltypes')
        assert isinstance(table, ir.TableExpr)
        assert_equal(table.schema(), fa.schema())
    ...
    def test_get_schema(self):
        t = self.con.table('functional_alltypes')
        schema = self.con.get_schema('functional_alltypes',
    ...
    def test_verbose_log_queries(self):
        ...
        with config.option_context('verbose', True):
            with config.option_context('verbose_log', logger):
                self.con.table('functional_alltypes',
                               database=self.test_data_db)

        assert len(queries) == 1
        expected = 'DESC {0}.`functional_alltypes`'.format(self.test_data_db)
