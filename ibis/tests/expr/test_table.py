...
def test_getitem_missing_column(table):
    ...
    assert excinfo.value.container is table
    assert excinfo.value.suggestions == {"oops": ()}
    ...
    assert excinfo.value.existing_options == tuple(table.columns)
    assert excinfo.value.container is table
    assert excinfo.value.suggestions == {"A": ("a",)}
...
def test_select_missing_columns(table):
    ...
    assert excinfo.value.existing_options == tuple(table.columns)
    assert excinfo.value.container is table
    assert excinfo.value.suggestions == {"B": ("b",), "bar": (), "qux": (), "foo": ()}
