...
def is_nan_like(col: ir.Value) -> ir.BooleanValue:
    if not col.type().is_string():
        return col.isnull()
    result = col.isnull()
    result |= col.lower().isin(NAN_LIKE).fillnull(False)
    result |= ~contains_alphanum(col).fillnull(False)
    return result
...
def to_ascii(s: ir.StringValue) -> ir.StringValue:
    ...
    # return norm_whitespace(s.fillnull("").apply(unidecode).astype(s.dtype))
...
def num_tokens(s: ir.StringValue) -> ir.IntegerValue:
    ...
    s = s.strip()
    s = s.nullif("")
    return s.split(" ").length().fillnull(0)
...
def choose_longer(s1: ir.StringColumn, s2: ir.StringColumn) -> ir.StringColumn:
    l1 = s1.length().fillnull(0)
    l2 = s2.length().fillnull(0)
...
def parse_middle(
    ...
    middle_is_middle = (starts_with(middle, b) | starts_with(b, middle)).fillnull(False)
    result_first = middle_is_middle.ifelse(a, first)
    result_middle = middle_is_middle.ifelse(choose_longer(b, middle), middle)

    al = a.length().fillnull(0)
    bl = b.length().fillnull(0)
    ...
    first_is_last = starts_with(last, a).fillnull(False)
...
def fix_nickname_is_middle(t: ir.Table) -> ir.Table:
    ...
    """
    todo = starts_with(t["nickname"], t["middle_name"]).fillnull(False)
...
def fix_last_comma_first(t: ir.Table) -> ir.Table:
    ...
    first_empty = t.first_name.strip().fillnull("") == ""
