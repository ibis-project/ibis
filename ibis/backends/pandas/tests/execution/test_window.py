...
execute = ibis.backends.pandas.execute
...
def test_mutate_with_window_after_join(sort_kind):
    ...
    con = ibis.backends.pandas.connect(dict(left=left_df, right=right_df))
...
def test_project_list_scalar():
    ...
    con = ibis.backends.pandas.connect(dict(df=df))
    expr = con.table('df')
...
def test_window_with_preceding_expr(index):
    ...
    client = ibis.backends.pandas.connect({'df': df})
    t = client.table('df')
    expected = (
        df.set_index('time')
...
def test_window_has_pre_execute_scope():
    signature = ops.Lag, ibis.backends.pandas.PandasClient
    called = [0]
    ...
    client = ibis.backends.pandas.connect(dict(df=df))
    t = client.table('df')
