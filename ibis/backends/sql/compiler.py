...
class SQLGlotCompiler(abc.ABC):
    ...
    def visit_DefaultLiteral(self, op, *, value, dtype):
        ...
        if dtype.is_integer():
            ...
            items = [
                ...
                ).as_(k, quoted=self.quoted, copy=False)
    ...
    def visit_StructField(self, op, *, arg, field):
        return sge.Dot(
            this=arg, expression=sg.to_identifier(field, quoted=self.quoted, copy=False)
        )
    ...
    def visit_Argument(self, op, *, name: str, shape, dtype):
        return sg.to_identifier(op.param, copy=False)
    ...
    def _dedup_name(
        ...
        if isinstance(value, sge.Column) and key == value.name:
            ...
            return value.as_(key, quoted=self.quoted)
    ...
    def visit_FillNa(self, op, *, parent, replacements):
        ...
        exprs = {
            col: (
                self.f.coalesce(
                    sg.column(col, quoted=self.quoted, copy=False), sge.convert(alt)
                )
    ...
    def visit_View(self, op, *, child, name: str):
        ...
        try:
            ...
            return child.as_(name, copy=False)
