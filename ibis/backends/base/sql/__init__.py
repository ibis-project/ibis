...
# XXX: Temporary making `BaseSQLBackend` not subclass `BaseBackend`
# While making backends stop using `Client`, we need to be able to
# subclass `BaseSQLBackend` without `BaseBackend` in `Client` classes.
# This is because `BaseBackend` has abstract methods not implemented
# in legacy `Client` classes.
# `Backend` classes need to subclass both, but instead of subclassing
# `BaseBackend` through `BaseSQLBackend`, they can inherit from both
# for now.
