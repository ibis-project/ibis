...
# TODO - update the rest of the docs here
...
Note that in the dask backend, only trailing and cumulative windows are
supported right now.
...
::
    ...
    ...     'time': date_range(periods=4, start='now')
    ... })
    >>> s = dd.Series(df.value.sum(), index=df.index, name='sum_value')
    >>> s  # doctest: +SKIP
...
class Summarize(AggregationContext):
    ...
    def agg(self, grouped_data, function, *args, **kwargs):
        ...
        if isinstance(function, dd.Aggregation):
...
def compute_window_spec_interval(_, expr):
    value = ibis.dask.execute(expr)
    return tseries.frequencies.to_offset(value)
...
def create_window_input_iter(
    grouped_data: Union[SeriesGroupBy, dd.Series],
    masked_window_lower_indices: dd.Series,
    masked_window_upper_indices: dd.Series,
) -> Iterator[np.ndarray]:
...
def window_agg_udf(
    ...
    function: Callable,
    window_lower_indices: dd.Series,
    window_upper_indices: dd.Series,
    mask: dd.Series,
    result_index: dd.Index,
    dtype: np.dtype,
    max_lookback: int,
    *args: Tuple[Any],
    **kwargs: Dict[str, Any],
    ...
    # way to pass multiple input dd.Series/SeriesGroupBy
...
class Moving(Window):
    ...
    def __init__(self, preceding, max_lookback, *args, **kwargs):
        ...
        closed = (
            None
            if not isinstance(
                preceding, timedelta_types + (tseries.offsets.DateOffset,)
            )
            else 'both'
        )
        super().__init__(
