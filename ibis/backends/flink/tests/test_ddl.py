...
def test_temporal_join(data_dir, con, tempdir_sink_configs):
    ...
    # temp_file = next(iter(os.listdir(table_left_tempdir.name)))
    # with open(os.path.join(table_left_tempdir.name, temp_file)) as f:
    #     file_data = f.read()
    #     print(f"file_data= \n{file_data}")
    ...
    sql = con.compile(expr)
    expected_sql = (
"""SELECT t0.`id`, t0.`bool_col`, t0.`smallint_col`, t0.`int_col`,
       t0.`timestamp_col`, t1.`bool_col` AS `bool_col_right`,
...
FROM table_left AS t0
  JOIN table_right FOR SYSTEM_TIME AS OF t0.`timestamp_col` AS t1
    ...
    expected_df_to_str = (
        "  id  bool_col  smallint_col  int_col           timestamp_col  bool_col_right  smallint_col_right  int_col_right     timestamp_col_right\n"
        "6690      True             0        0 2010-11-01 00:00:00.000            True                   0              0 2010-11-01 00:00:00.000\n"
        "6691     False             1        1 2010-11-01 00:01:00.000           False                   1              1 2010-11-01 00:01:00.000\n"
        "6692      True             2        2 2010-11-01 00:02:00.100            True                   2              2 2010-11-01 00:02:00.100\n"
        "6693     False             3        3 2010-11-01 00:03:00.300           False                   3              3 2010-11-01 00:03:00.300\n"
        "6694      True             4        4 2010-11-01 00:04:00.600            True                   4              4 2010-11-01 00:04:00.600"
    )
