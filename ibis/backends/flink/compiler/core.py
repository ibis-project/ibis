...
class FlinkTableSetFormatter(TableSetFormatter):
    ...
    def get_result(self):
        ...
        print(
            "Before append: \n"
            f"join_tables= {self.join_tables}"
        )
        if isinstance(op, ops.Join):
            self._walk_join_tree(op)
        else:
            # TODO (mehmet): What does this do exactly?
            # Could not get it exercised with the unit tests.
            self.record_join_tables(op)
        print(
            f"join_tables= {self.join_tables} \n"
            f"join_table_aliases= {self.join_table_aliases} \n"
        )
        ...
        for jtype, table, alias, preds in zip(
                self.join_types, self.join_tables[1:],
                self.join_table_aliases[1:], self.join_predicates
        ):
            print(
                f"jtype= {jtype} \n"
                f"table= {table} \n"
            ...
            if jtype == "JOIN":
                ...
                if self._format_table(op=asof.table, no_alias=True) != self.join_tables[0]:
                    raise com.UnsupportedArgumentError(
