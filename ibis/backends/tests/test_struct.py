...
def test_field_overwrite_always_prefers_unpacked(con):
    t = con.table("struct").filter(lambda t: t.abc.a.notnull())
    t1 = t.mutate(a=1)
    t2 = t1.select("a", "abc")
    t3 = t1.select("abc", "a")
    t4 = t2.select("abc", "a")
    ...
    assert set(con.execute(expr1).a) == expected
    assert set(con.execute(expr2).a) == expected
    assert set(con.execute(expr3).a) == expected
    assert set(con.execute(expr4).a) == expected
