  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        ...
        INNER JOIN "tpch"."lineitem" AS "t6"
          ON "t5"."s_suppkey" = "t6"."l_suppkey"
        INNER JOIN "tpch"."orders" AS "t7"
          ON "t7"."o_orderkey" = "t6"."l_orderkey"
        INNER JOIN "tpch"."customer" AS "t8"
          ON "t8"."c_custkey" = "t7"."o_custkey"
        INNER JOIN "tpch"."nation" AS "t9"
          ON "t5"."s_nationkey" = "t9"."n_nationkey"
        INNER JOIN "tpch"."nation" AS "t10"
      ...
      WHERE
        ...
        AND "t11"."l_shipdate" BETWEEN DATE_TRUNC('DAY', '1995-01-01') AND DATE_TRUNC('DAY', '1996-12-31')
    ) AS "t12"
  ) AS t12
  GROUP BY
    "t12"."supp_nation",
