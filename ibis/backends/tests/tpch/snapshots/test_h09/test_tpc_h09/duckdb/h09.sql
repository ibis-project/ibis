  ...
  FROM (
    ...
    FROM (
      ...
      INNER JOIN "supplier" AS t1
        ON (
          t1.s_suppkey = t0.l_suppkey
        )
      INNER JOIN "partsupp" AS t2
        ON (
          t2.ps_suppkey = t0.l_suppkey
        ) AND (
          t2.ps_partkey = t0.l_partkey
        )
      INNER JOIN "part" AS t3
        ON (
          t3.p_partkey = t0.l_partkey
        )
      INNER JOIN "orders" AS t4
        ON (
          t4.o_orderkey = t0.l_orderkey
        )
      INNER JOIN "nation" AS t5
        ON (
          t1.s_nationkey = t5.n_nationkey
        )
    ) AS t11
    WHERE
