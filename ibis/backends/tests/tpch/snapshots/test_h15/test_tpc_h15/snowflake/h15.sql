SELECT
  "t10"."s_suppkey" AS "s_suppkey",
  "t10"."s_name" AS "s_name",
  "t10"."s_address" AS "s_address",
  "t10"."s_phone" AS "s_phone",
  "t10"."total_revenue" AS "total_revenue"
FROM (
  SELECT
    *
  FROM (
    ...
    FROM (
      ...
      INNER JOIN (
        ...
        FROM (
          ...
          WHERE
            (
              "t3"."l_shipdate" >= DATEFROMPARTS(1996, 1, 1)
            )
            AND (
              "t3"."l_shipdate" < DATEFROMPARTS(1996, 4, 1)
            )
        ) AS "t4"
        GROUP BY
          1
