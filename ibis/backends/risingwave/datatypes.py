...
class RisingwaveType(AlchemyType):
    dialect = "risingwave"
    ...
    def from_ibis(cls, dtype: dt.DataType) -> sat.TypeEngine:
        if dtype.is_floating():
            ...
            if not (dtype.key_type.is_string() and dtype.value_type.is_string()):
                raise TypeError(
                    f"Risingwave only supports map<string, string>, got: {dtype}"
                )
            return psql.HSTORE()
        elif dtype.is_uuid():
            return psql.UUID()
    ...
    def to_ibis(cls, typ: sat.TypeEngine, nullable: bool = True) -> dt.DataType:
        if dtype := _from_postgres_types.get(type(typ)):
            ...
            if (unit := _postgres_interval_fields.get(field)) is None:
                ...
                raise ValueError(
                    "Variable length intervals are not yet supported with Risingwave"
                )
    ...
    @classmethod
    def from_string(cls, type_string: str) -> RisingwaveType:
