...
# translate strftime spec into mostly equivalent Risingwave spec
...
def _arbitrary(t, op):
    if (how := op.how) == "heavy":
        raise com.UnsupportedOperationError(
            f"risingwave backend doesn't support how={how!r} for the arbitrary() aggregate"
        )
    func = getattr(sa.func, op.how)
    return t._reduction(func, op)


class rw_struct_field(GenericFunction):
    inherit_cache = True


@compiles(rw_struct_field)
...
def _struct_field(t, op):
    ...
    return rw_struct_field(
        t.translate(arg), field_name, type_=t.get_sqla_type(op.dtype)
    )
...
def _struct_column(t, op):
    ...
    return sa.func.row(
        # we have to cast here, otherwise risingwave refuses to allow the statement
