...
class Backend(SQLBackend, CanCreateDatabase, UrlFromPath, PyArrowExampleLoader):
    ...
    def _register_in_memory_table(self, op: ops.InMemoryTable) -> None:
        return self._memtables_manager.register_in_memory_table(op)

    def _make_memtable_finalizer(self, name: str) -> Callable[..., None]:
        return self._memtables_manager.make_memtable_finalizer(name)

    class MemtablesManager:
        ...
        def _ensure_path_for_memtable(self, name: str) -> str:
            vol_path = self._get_volume_path()
            if not self._is_volume_created:
                self._create_volume(vol_path)
                self._is_volume_created = True
            return f"{vol_path}/{name}.parquet"

        def _get_volume_path(self) -> str:
            """Either returns the volume_path passed in __init__, or generates a unique one and caches it for future use.

            Does NOT actually create the volume in the backend, just returns the path where the volume should be.
            """
    ...
    def do_connect(
        ...
        **config: Any,
        ...
        self._memtables_manager = self.MemtablesManager(self, memtable_volume)
    ...
    def from_connection(cls, con, /, *, memtable_volume: str | None = None) -> Backend:
        ...
        memtable_volume
            ...
            This parameter specifies the volume to use for that storage, eg "/Volumes/my_volume".
        ...
        new_backend.con = con
        new_backend._memtables_manager = new_backend.MemtablesManager(
            new_backend, memtable_volume
        )
        return new_backend
