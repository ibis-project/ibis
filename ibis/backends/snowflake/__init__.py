...
$$ {defn["source"]} $$"""
    ...
    def _get_udf_source(self, udf_node: ops.ScalarUDF):
        ...
        ibis_version = ibis.__version__
        return dict(
            source=source,
            name=name,
            signature=signature,
            return_type=return_type,
            comment=f"Generated by ibis {ibis_version} using Python {python_version}",
        )
...
LANGUAGE PYTHON
IMMUTABLE
RUNTIME_VERSION = '3.8' -- Python UDFs are limited to Python 3.8
COMMENT = '{comment}'
HANDLER = '{name}'
AS $$
from __future__ import annotations
