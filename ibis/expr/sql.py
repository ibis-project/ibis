...
def to_sql(
    ...
    **kwargs,
    ...
    if isinstance(x, (dt.DataType, sch.Schema)):
        dialect = dialect or ibis.get_backend().dialect
        return SQLString(
            x.to_sqlglot(dialect=dialect).sql(dialect=dialect, pretty=pretty)
        )
    ...
    sql = ";\n".join(
        query.sql(dialect=compiler.dialect, pretty=pretty) for query in queries
    )
