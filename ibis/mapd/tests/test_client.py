...
def test_database_layer(client):
    mapd_dataset = client._proxy.get_dataset(client.dataset_id)
    actual = client.list_tables()
    expected = [el.name for el in mapd_dataset.list_tables()]
...
def test_compile_toplevel():
    ...
    # FIXME: remove quotes because mapd can't use anythig that needs
    # quoting?
    expected = """\
