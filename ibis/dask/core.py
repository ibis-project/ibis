...
from ibis.dask.dispatch import (
    execute_literal,
    execute_node,
    post_execute,
    pre_execute,
)
from ibis.dask.trace import trace
...
timestamp_types = Timestamp, datetime.datetime, np.datetime64
timedelta_types = Timedelta, datetime.timedelta, np.timedelta64
temporal_types = date_types + time_types + timestamp_types + timedelta_types
scalar_types = fixed_width_types + temporal_types + (dd.core.Scalar,)
...
def execute_with_scope(
    ...
    **kwargs,
    ...
    scope : Scope
        ...
        data such as a dask.dataframe DataFrame.
    timecontext : Optional[TimeContext]
        A tuple of (begin, end) that is passed from parent Node to children
        see [timecontext.py](ibis/dask/execution/timecontext.py) for
