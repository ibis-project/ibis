...
from ibis.dask.aggcontext import Window
...
def rule_to_python_type(datatype):
    """Convert an ibis :class:`~ibis.expr.datatypes.DataType` into a dask
    backend friendly ``multipledispatch`` signature.

    Parameters
    ----------
    rule : DataType
        The :class:`~ibis.expr.datatypes.DataType` subclass to map to a dask
        friendly type.

    Returns
    -------
    Union[Type[U], Tuple[Type[T], ...]]
        A dask-backend-friendly signature
    """
...
def pre_execute_analytic_and_reduction_udf(op, *clients, scope=None, **kwargs):
    ...
    def execute_udaf_node_groupby(op, *args, **kwargs):
        ...
        # for every argument excluding the first (dask performs
