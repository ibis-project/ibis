...
from ibis.dask.aggcontext import AggregationContext, window_agg_udf
from ibis.dask.dispatch import pre_execute
from ibis.dask.execution.window import get_aggcontext
from ibis.udf.vectorized import reduction

execute = ibis.dask.execute
...
class CustomAggContext(AggregationContext):
    ...
    def agg(self, grouped_data, function, *args, **kwargs):
        upper_indices = dd.Series(range(1, len(self.parent) + 2))
        window_sizes = (
    ...
    left_df = dd.DataFrame(
        {
            'ints': [0, 1, 2],
            'strings': ['a', 'b', 'c'],
            'dates': date_range('20170101', periods=3),
        }
    )
...
def test_mutate_scalar_with_window_after_join():
    left_df = dd.DataFrame({'ints': range(3)})
    right_df = dd.DataFrame(
        {
            'group': [0, 1, 2] * 3,
            'value': [0, 1, np.nan, 3, 4, np.nan, 6, 7, 8],
...
def test_project_scalar_after_join():
    ...
    result = expr.execute()
    expected = dd.DataFrame({'sum': [29.0] * 9, 'const': [1] * 9})
    tm.assert_frame_equal(result[expected.columns], expected)


def test_project_list_scalar():
    df = dd.DataFrame({'ints': range(3)})
    con = ibis.pandas.connect(dict(df=df))
    expr = con.table('df')
    result = expr.mutate(res=expr.ints.quantile([0.5, 0.95])).execute()
    tm.assert_series_equal(
        result.res, dd.Series([[1.0, 1.9] for _ in range(0, 3)], name='res')
    )
...
def test_window_with_preceding_expr(index):
    time = date_range('20180101', '20180110')
    start = 2
    data = np.arange(start, start + len(time))
    df = dd.DataFrame({'value': data, 'time': time}, index=index(time))
...
def test_window_with_mlb():
    index = date_range('20170501', '20170507')
    data = np.random.randn(len(index), 3)
    df = (
        dd.DataFrame(data, columns=list('abc'), index=index)
        .rename_axis('time')
...
def test_window_has_pre_execute_scope():
    ...
    df = dd.DataFrame(data, columns=['key', 'value', 'dup'])
...
def test_custom_window_udf(t, custom_window):
    ...
    expected = dd.Series([4.0, 10.0, 5.0])
