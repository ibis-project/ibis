...
from ibis.dask.execution.arrays import collect_list
...
def test_array_collect(t, df):
    ...
    expected = (
        ...
        .agg({'float64_with_zeros': collect_list})
        .rename(columns={'float64_with_zeros': 'collected'})
    )
    tm.assert_frame_equal(result.compute(), expected.compute())


@pytest.mark.xfail(raises=AttributeError, reason='TODO - figure out windows')
