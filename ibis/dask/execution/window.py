"""Code for computing window functions with ibis and dask."""
...
from ibis.dask.aggcontext import AggregationContext
...
from ibis.dask.dispatch import execute_node, pre_execute
from ibis.dask.execution import util


def _post_process_empty(
    result: Any, parent: dd.DataFrame, order_by: List[str], group_by: List[str]
) -> dd.Series:
...
def _post_process_order_by(
    series, parent: dd.DataFrame, order_by: List[str], group_by: List[str]
) -> dd.Series:
...
def trim_with_timecontext(data, timecontext: Optional[TimeContext]):
    """ Trim data within time range defined by timecontext
        ...
        Params
        ------
        data: dd.Series with MultiIndex
        timecontext: Optional[TimeContext]

        Returns:
        ------
        a trimmed dd.Series with same Multiindex struct as data
...
@execute_node.register(ops.CumulativeMean, (dd.Series, SeriesGroupBy))
...
@execute_node.register(ops.CumulativeOp, (dd.Series, SeriesGroupBy))
...
@execute_node.register(ops.MinRank, (dd.Series, SeriesGroupBy))
...
@execute_node.register(ops.DenseRank, (dd.Series, SeriesGroupBy))
def execute_series_dense_rank(op, data, **kwargs):
    # TODO(phillipc): Handle ORDER BY
    return data.rank(method='dense', ascending=True).astype('int64') - 1


@execute_node.register(ops.PercentRank, (dd.Series, SeriesGroupBy))
